...
public static class GeoDbOptimize
{
        ...
        List<List<PointD>> areaFilter = null)
    {
        ...
        foreach (var f in featureClassFilter) sb.Append(f.ToFeatureCode());
    ...
    private static double Angle2D(PointD point1, PointD point2)
    {
        ...
        else if (delta < -Math.PI) delta += Math.PI + Math.PI;
        return delta;
    }
    ...
    {
        if (featureClassFilter.IndexOf(fields.GetValue(PlaceInfoFields.FeatureClass),
                StringComparison.OrdinalIgnoreCase) == -1) return false;

        if (countryCodeFilter.IndexOf(fields.GetValue(PlaceInfoFields.CountryCode),
                StringComparison.OrdinalIgnoreCase) == -1) return false;

        if (areaFilter == null) return false;
    ...
    private static bool PointInPolyManifold(PointD point, IReadOnlyList<PointD> poly)
    {
        double angle = 0;
        var size = poly.Count;
        for (var i = 0; i < size; i++) angle += Angle2D(poly[i] - point, poly[(i + 1) % size] - point);
