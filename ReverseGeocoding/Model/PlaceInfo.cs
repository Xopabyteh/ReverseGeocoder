...
public readonly record struct PlaceInfo : IKdComparator<PlaceInfo>, IPlaceInfo
{
    private const char k_FieldsDelimiter = '\t';
    public const int fieldCount = 19;
    private const string k_ModificationDateFormat = "yyyy-MM-dd";

    public PlaceInfo(string geoEntry, IReadOnlyDictionary<string, ICountryInfo>? countryInfos)
    {
        var fields = geoEntry.Split(k_FieldsDelimiter);
        if (fields.Length != fieldCount) throw new ArgumentException("Invalid GeoName Record");
        ...
        ModificationDate = DateTime.ParseExact(fields.GetValue(PlaceInfoFields.ModificationDate),
            k_ModificationDateFormat, CultureInfo.InvariantCulture);

        if (countryInfos is not null &&
            !string.IsNullOrWhiteSpace(CountryCode) &&
            countryInfos.TryGetValue(CountryCode, out var info))
        {
            CountryInfo = info;
        }
    ...
    public ICountryInfo CountryInfo { get; init; }
