...
internal class KdTree<T> where T : IKdComparator<T>
{
    ...
    private KdNode<T> CreateTree(T[] items, int start, int end, int depth)
    {
        ...
        return new KdNode<T>(CreateTree(items, start, index, depth + 1), CreateTree(items, index + 1, end, depth + 1),
            items[index]);
    }
    ...
    private KdNode<T> FindNearest(KdNode<T> node, T dearch, int depth)
    {
        ...
        if (node.Location.SquaredDistance(dearch) < best.Location.SquaredDistance(dearch)) best = node;
